...
enum class States { IDLE, OFF, ON };

enum class Events { START, ON_TIMER1 };

enum class Timers { TIMER1 = 1 };
...
int main(const int argc, const char** argv) {
    ...
    hsm.registerState(States::IDLE, [&hsm](const VariantVector_t& args) { printf("Idle\n"); });
    hsm.registerState(States::OFF, [&hsm](const VariantVector_t& args) { printf("Off\n"); });
    hsm.registerState(States::ON, [&hsm](const VariantVector_t& args) { printf("On\n"); });
